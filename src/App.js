...
function App() {
  ...
  const [isLoading, setIsingLoading] = useState(false);
...
function FactList({ facts, setFacts }) {
  if (facts.length === 0)
    return (
      <p className="message">
        No facts for this category yet!Create the first！
      </p>
    );
...
function Fact({ fact, setFacts }) {
  ...
  const isDisputed =
    fact.voteintresting + fact.votemindblowing < fact.votefalse;
  ...
  return (
    <li className="fact">
      ...
      <div className="vote-buttons">
        <button
          onClick={() => handleVote("voteintresting")}
          disabled={isUpdating}
        >
          👍{fact.voteintresting}
        </button>
